  ...
  <q-layout view="hHh lpr lFf" class="main-layout-container">
    <q-header v-if="router.currentRoute.value.path !== '/auth'" style="backgroundColor:white" elevated>
      ...
      <q-toolbar style="height:54px">
        ...
        <img v-else :src="logo" @click="router.push({ path: '/' })" class="severstal-logo cursor-pointer" />
        <q-space />
        ...
        <div>
          <div class="row nav-routes">
            ...
            <div @mouseover="setUnderline(1)" @mouseleave="dropUnderline()" @click="shown = !shown">
              Анализ
            </div>
            <div @mouseover="setUnderline(2)" @mouseleave="dropUnderline()" @click="router.push({ name: 'Download' })">
              Загрузка</div>
          </div>
